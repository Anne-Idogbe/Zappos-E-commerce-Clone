...
const AddProduct = () => {
  ...
  const [brand, setBrand] = useState("Zappos");
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        const storedCategories = JSON.parse(localStorage.getItem("createdCategory")) || [];
  ...
  return (
    <div className="max-w-md mx-auto mt-10 p-6 bg-white rounded-lg shadow-md min-h-screen">
      ...
      {showProductForm && (
        <div className="max-w-2xl mx-auto mt-10 p-6 bg-white rounded-lg shadow-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* Shipping Locations */}
            <div>
              <label htmlFor="productShippingLoc" className="block text-sm font-medium text-gray-700">Shipping Locations</label>
              <select
                id="productShippingLoc"
                multiple
                value={shippingLoc}
                onChange={(e) => setShippingLoc([...e.target.selectedOptions].map(option => option.value))} // Map selected options to an array
                className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
              >
                {/* Example shipping locations */}
                <option value="USA">Nigeria</option>
                <option value="Canada">Ghana</option>
                <option value="UK">Cameroun</option>
